...
Официальная метрика соревнования — **RMSE в log-пространстве**:

```
rmse_log = sqrt(mean((log(pred+1) - log(true+1))^2))
```

Чем меньше — тем лучше. Штрафует за относительную ошибку, а не абсолютную — промахнуться на 10% при любой цене одинаково плохо.

В выводе также показывается `rmse` в реальных долларах — для понимания масштаба ошибки.
